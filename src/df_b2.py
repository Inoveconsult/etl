...
df_set = criar_dataframe(
    origem=dados +"\B2_092025.xlsx",    
    skiprows=17)    

df_out = criar_dataframe(
    origem=dados +"\B2_102025.xlsx",    
    skiprows=17)

df_nov = criar_dataframe(
    origem=dados +"\B2_112025.xlsx",    
    skiprows=17)

df_dez = criar_dataframe(
    origem=dados +"\B2_122025.xlsx",    
    skiprows=17)

colunas=["CNES","ESTABELECIMENTO","TIPO DO ESTABELECIMENTO","SIGLA DA EQUIPE",
         "Nº TOTAL DE PESSOAS COM TRATAMENTO ODONTOLÓGICO CONCLUÍDO PELA ESB",         
...
def gerar_df_b2():
    ...
    df = calcular_nota_final(df, coluna_conceito="CONCEITO", peso=2, nome_coluna_saida="NOTA B2")
    
    df = ordenar_por_nota_e_quadrimestre(df, coluna_nota="NOTA B2")
